...
const Profile = () => {
  ...
  const getData = async () => {
    ...
    axios.get("http://localhost:5000/useauth")
      ...
      <section className="flex flex-col items-center justify-center h-screen  -my-20 md:-mt-48 px-8">
        <div className="flex flex-col items-center justify-center text-center h-screen-half">
          <img
            ...
            alt={profile.photo}
          />
          <h1 className="text-5xl sm:text-6xl lg:text-9xl">{profile.fullname}</h1>
          <h2 className="font-light text-4xl sm:text-5xl lg:text-6xl">
            {profile.numberid}
          </h2>
        </div>
      </section>
